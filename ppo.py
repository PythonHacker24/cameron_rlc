...
═══════════════════════════════════════════════════════
  ...
  Reward (raw)   r = +1 every timestep the pole stays upright
  Reward (shaped) r' = r − α·x² − β·θ²
      α penalises cart drift from centre
      β penalises pole angle from vertical
  Done   when |θ| > 12° OR |x| > 2.4 m   (or 500 steps)

  Goal   keep the pole balanced AND the cart centred (max raw reward = 500)
"""
...
def draw_cartpole(ax, state, title="", color_pole="#FF6B35", color_cart="#2EC4B6"):
    ...
    # Wheels
    for dx in [-0.17, 0.17]:
        wheel = plt.Circle((x + dx, -cart_h / 2 - 0.07), 0.07, color="#555", zorder=4)
        ax.add_patch(wheel)
    ...
    # Bob
    ax.scatter(
        [px], [py], s=120, color="#FFE66D", zorder=6, edgecolors="white", linewidths=1
    )
...
for ax, theta0 in zip(axes, test_angles):
    draw_cartpole(ax, [0, 0, theta0, 0], title=f"θ₀ = {np.degrees(theta0):.1f}°")

fig.suptitle(
    "CartPole-v1 — Initial Conditions", color="white", fontsize=13, fontweight="bold"
)
plt.tight_layout()
plt.savefig(
    "01_environment_overview.png", dpi=120, bbox_inches="tight", facecolor="#0d1117"
)
plt.show()
...
# These don't change the task; they guide the agent to also centre the cart.
POS_COEF = 0.10  # penalty weight on x²   (cart distance from centre)
ANGLE_COEF = 5.0  # penalty weight on θ²   (pole angle from vertical)
...
for k, v in {
    ...
    "CLIP_EPS": CLIP_EPS,
    "LR": LR,
    "POS_COEF (reward shaping)": POS_COEF,
    "ANGLE_COEF (reward shaping)": ANGLE_COEF,
}.items():
    print(f"  {k:<30} = {v}")
...
print(
    f"{'Update':>7}  {'Episode':>8}  {'EpReward':>10}  "
...
for update in range(1, N_UPDATES + 1):
    ...
    for step in range(N_STEPS):
        ...
        # ── Reward shaping: penalise cart drift + pole angle ──
        x_pos, _, pole_angle, _ = next_state
        shaped_reward = reward - POS_COEF * x_pos**2 - ANGLE_COEF * pole_angle**2
        buf_rewards[step] = shaped_reward
    ...
    # ── PPO update ────────────────────────────────────────────────────
    m = ppo_update(
        model, optimizer, buf_states, buf_actions, buf_log_probs, advantages, returns
    )
    updates += 1
...
fig.suptitle(
    "PPO Training Curves — CartPole-v1",
    color="white",
    fontsize=15,
    fontweight="bold",
    y=0.98,
...
ax7.fill_between(range(len(log["avg100"])), log["avg100"], alpha=0.25, color="#2EC4B6")
ax7.plot(log["avg100"], color="#2EC4B6", linewidth=2)
ax7.axhline(495, color="#FF6B35", linestyle="--", alpha=0.7)
ax7.set_ylim(0, 520)
styled_ax(ax7, "Rolling Avg-100 Reward (Agent Skill Progression)", ylabel="Avg Reward")
...
def run_episode_from_theta(theta0, max_steps=400):
    ...
    for _ in range(max_steps):
        ...
        rewards.append(r)
        total_reward += r  # raw score (unshapen) for display
        if done:
            break
...
for t in test_thetas:
    ...
    final_x = states[-1][0]
    print(
        f"  θ₀ = {np.degrees(t):+6.1f}°  →  survived {len(states) - 1:>3} steps  "
        f"(score {score:.0f})  final x = {final_x:+.3f} m"
    )
...
plt.rcParams.update(style)
fig.suptitle(
    "PPO Stabilisation from Different Starting Angles θ₀",
    color="white",
    fontsize=14,
    fontweight="bold",
    y=0.98,
...
for ax, theta0 in zip(axes.flat, test_thetas):
    ...
    ax.plot(t_axis, cart_x, color="#A78BFA", linewidth=1.5, alpha=0.8, label="x (m)")
    ...
    final_x = states[-1][0]
    success_color = (
        "#34D399" if score >= 390 else ("#FBBF24" if score >= 200 else "#FF6B35")
    )
    ax.set_title(
        f"θ₀={np.degrees(theta0):+.1f}°  |  score={score:.0f}  |  x_f={final_x:+.2f}m",
        fontsize=9,
        color=success_color,
        pad=4,
    )
    ...
    ax.set_ylabel("Pole Angle (°) / Cart x (m)", fontsize=7, color="#888")
    ax.legend(
        fontsize=6,
        facecolor="#1a1a2e",
        edgecolor="#333",
...
plt.savefig(
    "03_angle_trajectories.png", dpi=130, bbox_inches="tight", facecolor="#0d1117"
)
plt.show()
print("✓ Saved: 03_angle_trajectories.png")


# ── Cell 10b ── Cart Position Trajectory (centering behaviour) ────────────────
fig_cx, axes_cx = plt.subplots(3, 3, figsize=(15, 11), facecolor="#0d1117")
plt.rcParams.update(style)
fig_cx.suptitle(
    "Cart Position x over Time — Does the Agent Centre Itself?",
    color="white",
    fontsize=14,
    fontweight="bold",
    y=0.98,
...
for ax, theta0 in zip(axes_cx.flat, test_thetas):
    data = results[theta0]
    states = data["states"]
    t_axis = np.arange(len(states)) * 0.02

    ax.set_facecolor("#111820")
    cart_x = states[:, 0]
    ax.plot(t_axis, cart_x, color="#A78BFA", linewidth=2.0, label="x (m)")
    ax.axhline(
        0, color="#34D399", linewidth=1.2, linestyle="--", alpha=0.8, label="Centre"
    )
    ax.axhline(2.4, color="#FF6B35", linewidth=1, linestyle=":", alpha=0.5)
    ax.axhline(-2.4, color="#FF6B35", linewidth=1, linestyle=":", alpha=0.5)
    ax.fill_between(t_axis, -0.3, 0.3, alpha=0.08, color="#34D399", label="±0.3 m zone")
    ...
    ax.set_title(
        f"θ₀={np.degrees(theta0):+.1f}°  |  x_final={final_x:+.2f} m",
        fontsize=9,
        color=clr,
        pad=4,
    )
    ...
    ax.set_ylim(-2.8, 2.8)
    ax.set_xlabel("Time (s)", fontsize=7, color="#888")
    ax.set_ylabel("Cart Position x (m)", fontsize=7, color="#888")
    ax.legend(
        fontsize=6,
        facecolor="#1a1a2e",
        edgecolor="#333",
...
plt.tight_layout(rect=[0, 0, 1, 0.96])
plt.savefig("03b_cart_centering.png", dpi=130, bbox_inches="tight", facecolor="#0d1117")
plt.show()
print("✓ Saved: 03b_cart_centering.png")
...
plt.tight_layout()
plt.savefig(
    "04_stabilisation_frames.png", dpi=130, bbox_inches="tight", facecolor="#0d1117"
)
plt.show()
...
plt.rcParams.update(style)
fig.suptitle(
    "Phase Portraits  (θ vs θ̇)  — PPO Driving State to Equilibrium",
    color="white",
    fontsize=13,
    fontweight="bold",
)
...
for ax, theta0 in zip(axes, selected):
    ...
    cart_x_vals = states[:, 0]

    ax.set_facecolor("#111820")
    sc = ax.scatter(
        theta_vals,
        thetadot_vals,
        c=np.arange(len(theta_vals)),
        cmap="plasma",
        s=8,
        alpha=0.9,
    )
    ...
    final_x = cart_x_vals[-1]
    ax.set_title(
        f"θ₀ = {np.degrees(theta0):+.1f}°  |  x_f={final_x:+.2f}m",
        color="white",
        fontsize=10,
    )
...
plt.tight_layout()
plt.savefig("05_phase_portraits.png", dpi=130, bbox_inches="tight", facecolor="#0d1117")
plt.show()
...
ax.set_facecolor("#111820")
colors = [
    "#FF6B35" if s < 200 else "#FBBF24" if s < 400 else "#34D399" for s in all_scores
]
bars = ax.bar(
    np.degrees(all_thetas),
    all_scores,
    width=0.7,
    color=colors,
    edgecolor="#222",
...
ax.axhline(200, color="#FBBF24", linestyle=":", linewidth=1, alpha=0.6, label="Partial")
ax.set_title("PPO Score vs Starting Angle θ₀", color="white", fontsize=12, pad=8)
...
plt.tight_layout()
plt.savefig("06_score_vs_angle.png", dpi=130, bbox_inches="tight", facecolor="#0d1117")
plt.show()
...
def render_frame(ax, state, theta0, frame_idx):
    ...
    ax.plot(
        [-2.4, -2.4], [-0.05, 0.05], color="#FF6B35", linewidth=2, alpha=0.6, zorder=0
    )
    ax.plot(
        [2.4, 2.4], [-0.05, 0.05], color="#FF6B35", linewidth=2, alpha=0.6, zorder=0
    )
    ...
    for dx in [-0.18, 0.18]:
        ax.add_patch(
            plt.Circle((x + dx, -cart_h / 2 - 0.08), 0.075, color="#145a52", zorder=4)
        )
        ax.add_patch(
            plt.Circle(
                (x + dx, -cart_h / 2 - 0.08),
    ...
    # Bob
    ax.scatter(
        [px], [py], s=130, color="#FFE66D", zorder=6, edgecolors="white", linewidths=1.2
    )
    ...
    ax.text(
        0,
        2.10,
        f"theta0={np.degrees(theta0):+.1f}deg",
        ha="center",
        va="top",
        fontsize=7,
        color="#aaa",
...
ani = FuncAnimation(fig_anim, animate, frames=frames, interval=1000 / FPS, blit=False)
...
plt.savefig(
    "07_simulation_snapshots.png", dpi=110, bbox_inches="tight", facecolor="#0d1117"
)
plt.show()
